...
def calculate_accumulated_rollover(user_id, category_id, current_month):
    """
    Накопленный перенос для категории на начало текущего месяца.
    Учитываем любые дельты (лимит - траты), в т.ч. ОТРИЦАТЕЛЬНЫЕ.
    """
    conn = get_db()
    try:
        rows = conn.execute("""
        ...
        carry = 0.0
        for r in rows:
            carry += float(r["limit_amount"]) - float(r["spent_amount"])
        return carry
    finally:
        conn.close()
...
def update_rollover_for_month(user_id, category_id, month, limit_amount, spent_amount):
    ...
    # Исходящий перенос = входящий + (лимит - потрачено)
    carry_out = accumulated_rollover + (limit_amount - spent_amount)
    
    try:
        # Используем INSERT OR REPLACE для обновления/создания записи (сохраняем исходящий перенос)
        conn.execute("""
            INSERT OR REPLACE INTO budget_rollover
            (user_id, category_id, month, limit_amount, spent_amount, rollover_amount, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP)
        """, (user_id, category_id, month, limit_amount, spent_amount, carry_out))
...
# -----------------------------------------------------------------------------
# Helper: Overall monthly rollover (not per-category)
# -----------------------------------------------------------------------------
def compute_overall_rollover(user_id: int, current_month: str):
    # Возвращает (carry_in, carry_out, available_with_carry) для текущего месяца.
    # carry_in — перенос с предыдущих месяцев (может быть отрицательным).
    # carry_out — доступный остаток текущего месяца, который уйдет в следующий (может быть < 0).
    # available_with_carry = income(current) - spent(current) + carry_in
    conn = get_db()
    try:
        # Собираем доходы по месяцам (<= текущего)
        ...
        income_by_month = {row["ym"]: float(row["total"]) for row in income_rows}
        
        # Собираем расходы по месяцам (<= текущего)
        expense_rows = conn.execute(
            """
            SELECT month as ym, COALESCE(SUM(amount), 0) as total
            FROM expenses
        ...
        expense_by_month = {row["ym"]: float(row["total"]) for row in expense_rows}
        
        # Определяем диапазон месяцев для прогонки переноса
        ...
        # Берем минимальный месяц из имеющихся данных, если их нет — current_month
        all_months = list(income_by_month.keys()) + list(expense_by_month.keys()) + [current_month]
        start_ym = min(all_months, key=lambda s: ym_to_dt(s))
        start_dt = ym_to_dt(start_ym)
        end_dt = ym_to_dt(current_month)
        ...
        while dt <= end_dt:
            ym = dt.strftime("%Y-%m")
            income = income_by_month.get(ym, 0.0)
            spent = expense_by_month.get(ym, 0.0)
            if ym == current_month:
                cur_carry_in = carry
            available = income - spent + carry
            carry = available  # перенос уходит в следующий месяц
            if ym == current_month:
                cur_available = available
            dt = add_month(dt)
        
        return (cur_carry_in, carry, cur_available)
    finally:
        conn.close()
...
def dashboard():
    ...
    carry_in, carry_out, available_with_carry = compute_overall_rollover(uid, month)
    return render_template(
        ...
        carry_in=carry_in,
        carry_out=carry_out,
        available_with_carry=available_with_carry,
    )
